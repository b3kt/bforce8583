...
char *								(*CMultiXOpenSSLInterface::sk_value)(const _STACK *, int)=NULL;
int										(*CMultiXOpenSSLInterface::sk_num)(const _STACK *)=NULL;
...
bool	CMultiXOpenSSLInterface::LoadDlls()
{
	...
	InitSSLFunction(sk_value	,(char *	(*)(const _STACK *, int))	,h2);
	InitSSLFunction(sk_num	,(int	(*)(const _STACK *))	,h2);
  ...
  { 
		...
		if((Error = SSL_get_verify_result(m_pSSL)) != X509_V_OK)
    {
		...
		if(m_bServerNameVerify)
		{
            ...
            if (!Method)
							break;
						/** NOTE: after OPENSSL 1.1.0 - You should replace extension->value->data with the ASN1_STRING_data(X509_EXTENSION_get_data(extension)) 
							and extension->value->length (twice) with the ASN1_STRING_length(X509_EXTENSION_get_data(extension))).
						 **/
						Data = ASN1_STRING_data(X509_EXTENSION_get_data(Ext));
#if (OPENSSL_VERSION_NUMBER > 0x00907000L)
						if(Method->it) 
							ExtData = ASN1_item_d2i(NULL, &Data, ASN1_STRING_length(X509_EXTENSION_get_data(Ext)), ASN1_ITEM_ptr(Method->it));
						else
						{ /** OpenSSL not perfectly portable at this point (?):
							...
							NOTE: after OPENSSL 1.1.0 -	
							You should replace extension->value->data with the ASN1_STRING_data(X509_EXTENSION_get_data(extension)) 
							and extension->value->length (twice) with the ASN1_STRING_length(X509_EXTENSION_get_data(extension))).
						  **/
							ExtData = Method->d2i(NULL, &Data, ASN1_STRING_length(X509_EXTENSION_get_data(Ext)));
						...
						else
						{
							...
							if(tmp)
							{
								if(!stricmp(m_RemoteHost.c_str(), (const char*)tmp))
									OK = 1;
								/** NOTE: disabled by bekt, after using openssl 1.1.1 **/	
								// OPENSSL_free(tmp);
							}
						}
					}
